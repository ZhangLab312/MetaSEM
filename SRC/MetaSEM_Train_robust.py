...
from SRC.MetaSEM_Model_robust import *
import copy
...
class Train_roubust:
    ...
    def train_model(self,input_path,net_path):
        ...
        #epoch
        best_Epr = 0
        best_AUPR = best_FPR = best_AUROC = 0
        for epoch in range(self.epoch):
            ...
            for idx in truth_edges:
                y0[idx] = 1
            main_net = Inference(num_genes, 64, num_genes, 3, 3).cuda()
            meta_net = MetaGRNInference(num_genes,3,adj_A_t.cpu(),y0,self.opt).cuda()
