...
class Inference(nn.Module):
    ...
    def forward(self,x):
        ...
        return logits
"""        mean, var = self.inference_get_mean(logits)
        logvar = torch.exp(var)
        Z = self.reparameterize(mean, logvar)
        output = {'mean': mean, 'var': logvar, 'gaussian': Z,
                  'logits': logits}"""


class MetaGRNInference(nn.Module):
    def __init__(self,num_genes,K,adj,y0,opt):
        super(MetaGRNInference, self).__init__()
        self.opt = opt
        self.K = K
        self.y0 = y0
        self.adj = nn.Parameter(Variable((adj).float(), requires_grad=True, name='adj_A'))
        self.weight = nn.Linear(num_genes, 64, bias=True)
        self.weight2 = nn.Linear(64, num_genes, bias=False)
        self.ys = []
        y = self.y0
